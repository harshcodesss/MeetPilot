"""Auth router: Google OAuth flow, /me, /auth/logout."""
...
async def google_callback(request: Request, db: Session = Depends(get_db)):
    ...
    to the frontend callback with ?token=<opaque-bearer>.

    Failure paths: OAuth-exchange failures (user denied consent on Google,
    state CSRF mismatch, etc.) are caught and redirected to the frontend
    callback with a short `?error=<code>` so the user sees the friendly
    "Sign-in failed" UI instead of a stack trace. DB / handler-body failures
    still 500 intentionally; they're internal bugs, not Google's fault.
    """
    try:
        ...
        return RedirectResponse(
            f"{AUTH_SUCCESS_REDIRECT_URL}?error={error_code}"
        )
    except Exception:
        # Network blip to Google's token endpoint, malformed response, etc.
        return RedirectResponse(
            f"{AUTH_SUCCESS_REDIRECT_URL}?error=sign_in_failed"
        )
